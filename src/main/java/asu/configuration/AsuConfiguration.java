...
/**
 * Конфигурация подключения к БД ASU
 * Date:   29.11.2018
 */
@Configuration
@EnableJpaRepositories(
    basePackages = "asu.repository",
    entityManagerFactoryRef = "asuEntityManagerFactory"
)
@ComponentScan("asu.dao")
public class AsuConfiguration {

    @Bean(name = "asuEntityManagerFactory")
    public LocalContainerEntityManagerFactoryBean getLocalContainerEntityManagerFactoryBean() {
        LocalContainerEntityManagerFactoryBean emf = new LocalContainerEntityManagerFactoryBean();
        emf.setPackagesToScan("asu.entity");
        emf.setDataSource(new JndiDataSourceLookup().getDataSource(BaseConstant.JNDI.ASU));

        JpaVendorAdapter vendorAdapter = new HibernateJpaVendorAdapter();
        emf.setJpaVendorAdapter(vendorAdapter);

        Properties jpaProperties = new Properties();
        jpaProperties.put("hibernate.dialect", "org.hibernate.dialect.MySQL5Dialect"); // SQL диалект
