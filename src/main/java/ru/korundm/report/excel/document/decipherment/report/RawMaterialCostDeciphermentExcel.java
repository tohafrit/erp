...
/**
 * Класс для формирования документа "Расшифровка затрат на сырье и материалы"
 * @author mazur_ea
 * Date:   30.10.2019
 */
public class RawMaterialCostDeciphermentExcel implements DeciphermentExcel {
    ...
    private int dataRowCount = 14; // счетчик строки с которой начинается заполнения данных по изделиям (фактически курсор)
    ...
    public Workbook generate(long deciphermentId) throws IOException {
        ...
        // Создания страницы
        Sheet sheet = workbook.createSheet(getMessage("rawMaterialCost.sheetName"));
        sheet.setZoom(70);
        ...
        // Колонтитулы
        sheet.getFooter().setCenter(getMessage("rawMaterialCost.footer.print.page") + HeaderFooter.page()); // Добавление нижнего колонтитула
        ...
        // Название формы
        {
            ...
            cell.setCellValue(getMessage("rawMaterialCost.header.formNumber"));
        ...
        // Шапка расшифровки
        {
            ...
            for (int rowNum = 2; rowNum < 5; rowNum++) {
                ...
                String value;
                if (rowNum == 2) {
                    value = getMessage("rawMaterialCost.header.part1");
                } else if (rowNum == 3) {
                    value = getMessage("rawMaterialCost.header.part2");
                } else {
                    ...
                    value = getMessage("rawMaterialCost.header.part3") + " " + productName;
                }
        ...
        // строка под шапкой
        {
            ...
            Cell cell = row.createCell(0);
            cell.setCellValue(getMessage("rawMaterialCost.header.part4"));
            sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), cell.getColumnIndex(), SHEET_CELL_WIDTH));
        ...
        // Этап
        {
            ...
            Cell cell = row.createCell(0);
            cell.setCellValue(getMessage("rawMaterialCost.header.stage"));
        ...
        // Шапка таблицы
        {
            ...
            // Установка значений
            Object[][] headerValues = {
                {8, 0, getMessage("rawMaterialCost.header.column.number")}, // № п/п
                {8, 1, getMessage("rawMaterialCost.header.column.code")}, // Код
                {8, 2, getMessage("rawMaterialCost.header.column.name")}, // Наименование
                {8, 3, getMessage("rawMaterialCost.header.column.codeOKP")}, // Код ОКП/ОКПД2
                {8, 4, getMessage("rawMaterialCost.header.column.codeEKPS")}, // Код ЕКПС (при наличии)
                {8, 5, getMessage("rawMaterialCost.header.column.FNN")}, // ФНН (при наличии)
                {8, 6, getMessage("rawMaterialCost.header.column.uom")}, // Единица измерения
                {8, 7, getMessage("rawMaterialCost.header.column.reportPeriod")}, // Отчетный период/период, предшествующий планируемому
                {9, 7, getMessage("rawMaterialCost.header.column.consumptionRate")}, // расход на единицу продукции
                {9, 9, getMessage("rawMaterialCost.header.column.umCostRate")}, // цена за единицу измерения (руб.)
                {9, 11, getMessage("rawMaterialCost.header.column.expenses")}, // затраты (руб.)
                {9, 13, getMessage("rawMaterialCost.header.column.priceJustification")}, // обоснование цены поставки
                {9, 15, getMessage("rawMaterialCost.header.column.supplier")}, // организация-поставщик (подрядчик, исполнитель)
                {10, 7, getMessage("rawMaterialCost.header.column.plan")}, // план
                {10, 8, getMessage("rawMaterialCost.header.column.fact")}, // факт
                {10, 9, getMessage("rawMaterialCost.header.column.plan")}, // план
                {10, 10, getMessage("rawMaterialCost.header.column.fact")}, // факт
                {10, 11, getMessage("rawMaterialCost.header.column.plan")}, // план
                {10, 12, getMessage("rawMaterialCost.header.column.fact")}, // факт
                {10, 13, getMessage("rawMaterialCost.header.column.srcDocuments")}, // первичные документы (номер и дата договора, протокола, счета, иное)
                {10, 14, getMessage("rawMaterialCost.header.column.priceDefineMethod")}, // метод определения цены
                {10, 15, getMessage("rawMaterialCost.header.column.supplierName")}, // наименование
                {10, 16, getMessage("rawMaterialCost.header.column.supplierINN")}, // ИНН
                {8, 17, getMessage("rawMaterialCost.header.column.applicablePriceIndex")}, // Применяемый индекс цен
                {8, 18, getMessage("rawMaterialCost.header.column.planPeriod", periodYear)}, // Планируемый период (год _____)
                {9, 18, getMessage("rawMaterialCost.header.column.normConsumptionRate")}, // норма расхода на единицу продукции
                {9, 19, getMessage("rawMaterialCost.header.column.umCostRate")}, // цена за единицу измерения (руб.)
                {9, 20, getMessage("rawMaterialCost.header.column.expenses")}, // затраты (руб.)
                {9, 21, getMessage("rawMaterialCost.header.column.priceJustification")}, // обоснование цены поставки
                {9, 23, getMessage("rawMaterialCost.header.column.supplier")}, // организация-поставщик (подрядчик, исполнитель)
                {10, 21, getMessage("rawMaterialCost.header.column.justifyDocuments")}, // обосновывающие документы
                {10, 22, getMessage("rawMaterialCost.header.column.priceDefineMethod")}, // метод определения цены
                {10, 23, getMessage("rawMaterialCost.header.column.supplierName")}, // наименование
                {10, 24, getMessage("rawMaterialCost.header.column.supplierINN")} // ИНН
            };
        ...
        // Всего по разделу 1 и 2, сырье и материалы
        {
            fontProperty.setFontHeightInPoints((short) 10);
            cellStyleProperty.setFontIndex(ExcelUtil.INSTANCE.defineFontIndex(workbook, fontProperty));
            cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.CENTER);
            ...
            // Всего по разделу 1 и 2
            Row row = sheet.createRow(12);
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                Cell cell = row.createCell(cellNum);
                if (cellNum == 2) {
                    cell.setCellValue(getMessage("rawMaterialCost.body.totalFirstSecondSection"));
                } else if (cellNum == 11) {
                    cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                }
                ExcelUtil.INSTANCE.mergeApply(cell, cellStyleProperty);
            }
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.CENTER);
                if (cellNum == 0) {
                    cell.setCellValue(getMessage("rawMaterialCost.body.numberRawAndMaterials"));
                } else if (cellNum == 2) {
                    cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                    cell.setCellValue(getMessage("rawMaterialCost.body.rawAndMaterials"));
                }
                ExcelUtil.INSTANCE.mergeApply(cell, cellStyleProperty);
            }
            cellStyleProperty.reset();
        ...
        // Строка итого для первого раздела
        {
            cellStyleProperty.reset();
            fontProperty.reset();
            fontProperty.setFontHeightInPoints((short) 10);
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                if (cellNum == 2) {
                    cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                    cell.setCellValue(getMessage("rawMaterialCost.body.totalSectionFirst"));
        ...
        // Раздел 2 - вспомогательные материалы, итого
        {
            fontProperty.setFontHeightInPoints((short) 10);
            cellStyleProperty.setVerticalAlignment(VerticalAlignment.CENTER);
            cellStyleProperty.setBorderTop(BorderStyle.THIN);
            ...
            for (int rowNum = 0, rowNumberDest = dataRowCount + 4; dataRowCount <= rowNumberDest; dataRowCount++, rowNum++) {
                ...
                for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                    ...
                    if (rowNum == 0) {
                        if (cellNum == 0) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.numberSupportMaterials"));
                        } else if (cellNum == 2) {
                            cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                            cell.setCellValue(getMessage("rawMaterialCost.body.supportMaterials"));
                        }
                    } else if (rowNum == 3) {
                        if (cellNum == 0 || cellNum == 2) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.anyMark"));
                        } else if (cellNum != 1 && cellNum != 10 && cellNum != 12) {
                            cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                        }
                    } else if (rowNum == 4) {
                        if (cellNum == 2) {
                            cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                            cell.setCellValue(getMessage("rawMaterialCost.body.totalSectionSecond"));
                        } else if (cellNum == 11) {
                            cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                        }
                    } else {
                        if (cellNum == 0) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.numberSupportMaterials") + rowNum);
                        } else if (cellNum != 1 && cellNum != 10 && cellNum != 12) {
                            cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                        }
                    }
        ...
        // Раздел 3 - возвратные отходы
        {
            ...
            for (int rowNum = 0, rowNumberDest = dataRowCount + 4; dataRowCount <= rowNumberDest; dataRowCount++, rowNum++) {
                ...
                for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                    ...
                    if (rowNum == 0) {
                        if (cellNum == 2) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.totalSectionThird"));
                        } else if (cellNum == 11) {
                            cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                        }
                    } else if (rowNum == 1) {
                        if (cellNum == 0) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.numberRecyclableWaste"));
                        } else if (cellNum == 2) {
                            cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                            cell.setCellValue(getMessage("rawMaterialCost.body.RecyclableWaste"));
                        }
                    } else if (rowNum == 4) {
                        if (cellNum == 0 || cellNum == 2) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.anyMark"));
                        ...
                        if (cellNum == 0) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.numberRecyclableWaste") + rowNum);
                        } else if (cellNum != 1 && cellNum != 10 && cellNum != 12) {
                            cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                        }
                    }
        ...
        // Окончание документа
        final int endDocStartRow = dataRowCount + 1;
        {
            ...
            Object[][] signMessages = {
                {endDocStartRow, List.of(
                    new Pair<>(3, getMessage("rawMaterialCost.footer.position1")),
                    new Pair<>(18, getMessage("rawMaterialCost.footer.position2"))
                )},
                ...
                {endDocStartRow + 3, List.of(
                    new Pair<>(3, getMessage("rawMaterialCost.footer.initialsDefinition")),
                    new Pair<>(18, getMessage("rawMaterialCost.footer.initialsDefinition"))
                )},
                {endDocStartRow + 4, List.of(
                    new Pair<>(3, getMessage("rawMaterialCost.footer.signDate", periodYear)),
                    new Pair<>(18, getMessage("rawMaterialCost.footer.signDate", periodYear))
                )}
            };
        ...
        if (deciphermentDataProduct.getUniqueNumber() != null) {
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                if (cellNum == 0) { // №№
                    ...
                    cellStyleProperty.setWrapText(Boolean.TRUE);
                    cell.setCellValue(deciphermentDataProduct.getName() + " " +  getMessage("rawMaterialCost.body.inComposition"));
                } else if (cellNum == 3) { // Код ОКП/ОКПД2
                    cell.setCellValue("-");
                } else if (cellNum == 4 || cellNum == 5) {
                    cell.setCellValue(getMessage("rawMaterialCost.markUnknown"));
                } else if (cellNum == 6) { // единица измерения
                    cell.setCellValue(getMessage("rawMaterialCost.body.thingUnitMeasure"));
            ...
            for (int rowCount = 0; rowCount < 2; rowCount++) {
                ...
                for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                    ...
                    if (cellNum == 2) {
                        ...
                        cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                        if (rowCount == 0) {
                            cell.setCellValue(getMessage("rawMaterialCost.body.totalOneProduct", 1));
                        } else {
                            cell.setCellValue(getMessage("rawMaterialCost.body.totalManyProduct", deciphermentDataProduct.getProductCount()));
                        }
                    } else if (cellNum == 20) {
                        cellStyleProperty.setDataFormat(workbook.createDataFormat().getFormat("#,##0.00"));
                        if (rowCount == 0) {
        ...
        for (var deciphermentDataComponent : deciphermentDataProduct.getComponentList()) {
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                if (cellNum == 0) { // №№
                    ...
                    if (invoiceData != null) {
                        cell.setCellValue(
                            getMessage("rawMaterialCost.body.invoice") + " "
                                + invoiceData.getName() + " "
    ...
    private String generateRowNumber(DeciphermentDataProduct deciphermentDataProduct) {
        if (deciphermentDataProduct != null) {
            ...
            return "1." + String.join( ".", rowNumberFormerList);
        }
        return "";
    }
}