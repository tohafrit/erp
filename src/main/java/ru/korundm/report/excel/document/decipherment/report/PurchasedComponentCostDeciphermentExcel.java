...
/**
 * Класс для формирования документа "Расшифровка затрат на покупные комплектующие изделия"
 * @author mazur_ea
 * Date:   16.09.2019
 */
public class PurchasedComponentCostDeciphermentExcel implements DeciphermentExcel {
    ...
    public Workbook generate(long deciphermentId) throws IOException {
        ...
        // Создания страницы
        Sheet sheet = workbook.createSheet(getMessage("purchasedComponentCost.sheetName"));
        sheet.setZoom(70);
        ...
        // Колонтитулы
        sheet.getFooter().setCenter(getMessage("purchasedComponentCost.footer.print.page") + HeaderFooter.page()); // Добавление нижнего колонтитула
        ...
        // Название формы
        {
            ...
            cell.setCellValue(getMessage("purchasedComponentCost.header.formNumber"));
        ...
        // Шапка расшифровки
        {
            ...
            for (int rowNum = 2; rowNum < 5; rowNum++) {
                ...
                String value;
                if (rowNum == 2) {
                    value = getMessage("purchasedComponentCost.header.part1");
                } else if (rowNum == 3) {
                    value = getMessage("purchasedComponentCost.header.part2");
                } else {
                    ...
                    value = getMessage("purchasedComponentCost.header.part3") + " " + productName;
                }
        ...
        // строка под шапкой
        {
            ...
            Cell cell = row.createCell(0);
            cell.setCellValue(getMessage("purchasedComponentCost.header.part4"));
            sheet.addMergedRegion(new CellRangeAddress(row.getRowNum(), row.getRowNum(), cell.getColumnIndex(), SHEET_CELL_WIDTH));
        ...
        // Этап
        {
            ...
            Cell cell = row.createCell(0);
            cell.setCellValue(getMessage("purchasedComponentCost.header.stage"));
        ...
        // Шапка таблицы
        {
            ...
            // Установка значений
            Object[][] headerValues = {
                {8, 0, getMessage("purchasedComponentCost.header.column.number")}, // № п/п
                {8, 1, getMessage("purchasedComponentCost.header.column.code")}, // Код
                {8, 2, getMessage("purchasedComponentCost.header.column.name")}, // Наименование
                {8, 3, getMessage("purchasedComponentCost.header.column.codeOKP")}, // Код ОКП/ОКПД2
                {8, 4, getMessage("purchasedComponentCost.header.column.codeEKPS")}, // Код ЕКПС (при наличии)
                {8, 5, getMessage("purchasedComponentCost.header.column.FNN")}, // ФНН (при наличии)
                {8, 6, getMessage("purchasedComponentCost.header.column.uom")}, // Единица измерения
                {8, 7, getMessage("purchasedComponentCost.header.column.reportPeriod")}, // Отчетный период/период, предшествующий планируемому
                {9, 7, getMessage("purchasedComponentCost.header.column.consumptionRate")}, // расход на единицу продукции
                {9, 9, getMessage("purchasedComponentCost.header.column.umCostRate")}, // цена за единицу измерения (руб.)
                {9, 11, getMessage("purchasedComponentCost.header.column.expenses")}, // затраты (руб.)
                {9, 13, getMessage("purchasedComponentCost.header.column.priceJustification")}, // обоснование цены поставки
                {9, 15, getMessage("purchasedComponentCost.header.column.supplier")}, // организация-поставщик (подрядчик, исполнитель)
                {10, 7, getMessage("purchasedComponentCost.header.column.plan")}, // план
                {10, 8, getMessage("purchasedComponentCost.header.column.fact")}, // факт
                {10, 9, getMessage("purchasedComponentCost.header.column.plan")}, // план
                {10, 10, getMessage("purchasedComponentCost.header.column.fact")}, // факт
                {10, 11, getMessage("purchasedComponentCost.header.column.plan")}, // план
                {10, 12, getMessage("purchasedComponentCost.header.column.fact")}, // факт
                {10, 13, getMessage("purchasedComponentCost.header.column.srcDocuments")}, // первичные документы (номер и дата договора, протокола, счета, иное)
                {10, 14, getMessage("purchasedComponentCost.header.column.priceDefineMethod")}, // метод определения цены
                {10, 15, getMessage("purchasedComponentCost.header.column.supplierName")}, // наименование
                {10, 16, getMessage("purchasedComponentCost.header.column.supplierINN")}, // ИНН
                {8, 17, getMessage("purchasedComponentCost.header.column.applicablePriceIndex")}, // Применяемый индекс цен
                {8, 18, getMessage("purchasedComponentCost.header.column.planPeriod", periodYear)}, // Планируемый период (год _____)
                {9, 18, getMessage("purchasedComponentCost.header.column.normConsumptionRate")}, // норма расхода на единицу продукции
                {9, 19, getMessage("purchasedComponentCost.header.column.umCostRate")}, // цена за единицу измерения (руб.)
                {9, 20, getMessage("purchasedComponentCost.header.column.expenses")}, // затраты (руб.)
                {9, 21, getMessage("purchasedComponentCost.header.column.priceJustification")}, // обоснование цены поставки
                {9, 23, getMessage("purchasedComponentCost.header.column.supplier")}, // организация-поставщик (подрядчик, исполнитель)
                {10, 21, getMessage("purchasedComponentCost.header.column.justifyDocuments")}, // обосновывающие документы
                {10, 22, getMessage("purchasedComponentCost.header.column.priceDefineMethod")}, // метод определения цены
                {10, 23, getMessage("purchasedComponentCost.header.column.supplierName")}, // наименование
                {10, 24, getMessage("purchasedComponentCost.header.column.supplierINN")} // ИНН
            };
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                if (cellNum == 2) {
                    ...
                    cell.setCellValue(getMessage("purchasedComponentCost.header.PKI"));
                } else if (cellNum == 3) {
        ...
        for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
            ...
            if (cellNum == 2) {
                cell.setCellValue(getMessage("purchasedComponentCost.body.total"));
        ...
        final int endDocStartRow = dataRowCount + 2;
        {
            ...
            Object[][] signMessages = {
                {endDocStartRow, List.of(
                    new Pair<>(3, getMessage("purchasedComponentCost.footer.position1")),
                    new Pair<>(18, getMessage("purchasedComponentCost.footer.position2"))
                )},
                ...
                {endDocStartRow + 3, List.of(
                    new Pair<>(3, getMessage("purchasedComponentCost.footer.initialsDefinition")),
                    new Pair<>(18, getMessage("purchasedComponentCost.footer.initialsDefinition"))
                )},
                {endDocStartRow + 4, List.of(
                    new Pair<>(3, getMessage("purchasedComponentCost.footer.signDate", periodYear)),
                    new Pair<>(18, getMessage("purchasedComponentCost.footer.signDate", periodYear))
                )}
            };
        ...
        if (deciphermentDataProduct.getUniqueNumber() != null) {
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                if (cellNum == 0) { // №№
                    ...
                    cellStyleProperty.setWrapText(Boolean.TRUE);
                    cell.setCellValue(deciphermentDataProduct.getName() + " " +  getMessage("purchasedComponentCost.body.inComposition"));
                } else if (cellNum == 3) { // Код ОКП/ОКПД2
                    cell.setCellValue("-");
                } else if (cellNum == 4 || cellNum == 5) {
                    cell.setCellValue(getMessage("purchasedComponentCost.markUnknown"));
                } else if (cellNum == 6) { // единица измерения
                    cell.setCellValue(getMessage("purchasedComponentCost.body.thingUnitMeasure"));
            ...
            for (int rowCount = 0; rowCount < 2; rowCount++) {
                ...
                for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                    ...
                    if (cellNum == 2) {
                        ...
                        cellStyleProperty.setHorizontalAlignment(HorizontalAlignment.LEFT);
                        if (rowCount == 0) {
                            cell.setCellValue(getMessage("purchasedComponentCost.body.totalOneProduct", 1));
                        } else {
                            cell.setCellValue(getMessage("purchasedComponentCost.body.totalManyProduct", deciphermentDataProduct.getProductCount()));
                        }
                    } else if (cellNum == 20) {
                        cellStyleProperty.setDataFormat(workbook.createDataFormat().getFormat("#,##0.00"));
                        if (rowCount == 0) {
        ...
        for (var deciphermentDataComponent : deciphermentDataProduct.getComponentList()) {
            ...
            for (int cellNum = 0; cellNum <= SHEET_CELL_WIDTH; cellNum++) {
                ...
                if (cellNum == 0) { // №№
                    ...
                    if (invoiceData != null) {
                        cell.setCellValue(
                            getMessage("purchasedComponentCost.body.invoice") + " "
                            + invoiceData.getName() + " "
